...
## Prey first recovery 100 to 210
...
plot(x = tset, y = X.simu2, #putting prey into y axis, timeseries along x
     ...
     xlim = c(0, 210), ylim = c(0, 70)) #bounds of axes
## plot predator
...
## plot prey
lines(x = tset2, y = X.simu5, #prey from this for() loop
      lwd = 2, col = Xcol) #visuals
## plot predator
lines(x = tset2, y = P.simu5, #predator for this for() loop in y axis
      lwd = 2, col = Pcol) #visuals
...
lines(x = c(t_Pstart, t_stopP), y = c(0, 0), 
      lwd = 10, col = Pcol)
text(x = 180, y = -.2, 'Predator being hunted', cex = .7)
  ...
  # assign the value of Y
	...
	for(i in 2:length(tset)){
	  # calculating change in time
		...
		# calculating change in population size
		dX <- ( r_x*X*(1-X/K_x) - a_x*P*X ) * dt #equation without hunting
		dP <- ( P*(c*(a_x*X+a_y*Y)-d_p)*(1-P/K_p)) * dt #equation without hunting
...
#add numerical values for prey/preds equlibrium
text(x = 60, y = 30, 'Prey equilibirum = 35', cex = .7)
text(x = 60, y = 5, 'Predator equilibirum = 1.2187e-09', cex = .7)

#print values for equlibrium of prey:
P.simu6[20000] #print value at last time step of predator
X.simu6[20000] #print value at last time step of predator
...
plot(x = tset2, y = X.simu4,
     type = 'l', las = 1, lwd = 2, col = Xcol,
     xlab = 'Time', ylab = 'Density', main = "PREY FIRST",
     ylim = c(0, 70))
