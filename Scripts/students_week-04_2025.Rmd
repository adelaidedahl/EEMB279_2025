...
# 
I0 <- 1		#initial conditions for infected (1 person is infected)

# 
S0 <- N - I0	#setting initial conditions for ppl susceptible 

# 
R0 <- 0 	#setting recovered to 0 beacuse no one has seen the disease
```
...
N.simu1 <- NaN*tset
N.simu1[1] <- N #sim for population size, inputting initial condition set up above

# repeat empty box for each empty stage 
I.simu1 <- NaN*tset
I.simu1[1] <- I0 #simulation for infected class, inputting initial condition set up above

# sucsptibility
S.simu1 <- NaN*tset
S.simu1[1] <- S0 #simulation for suscept., inputting initial condition set up above

# empty box for R
R.simu1 <- NaN*tset
R.simu1[1] <- 0 #simpulation for R, inputting initial condition set up above
```
...
```{r}
# 
for(i in 2:length(tset)){ #because we have initial conditions, we start at 2
  # 
	dt <- tset[i]-tset[i-1] #calc delta t (length of time steps)
	
	# simplify typing within for() loop 
	S <- S.simu1[i-1] #assigning "S" to be the place before i
	I <- I.simu1[i-1] #assigning "I" to be the place before i
	R <- R.simu1[i-1] #assigning "R" to be the place before i
	
	# inputting equations / assigning as dS, dI, dR, dN (single objects) that we can input further down (using equations from class...)
	dS <- (-beta*S*I)*dt
	dI <- (beta*S*I-gamma*I)*dt
	dR <- (gamma*I)*dt
	dN <- dS+dI+dR  #total of all classes
	
	#last step of for() loop, adding change to previous value 
	S.simu1[i] <- S + dS
	I.simu1[i] <- I + dI
	R.simu1[i] <- R + dR
	N.simu1[i] <- N + dN
...
```{r}
sd(N.simu1) #should be zero b/c no change
N.simu1[length(tset)] #pulling the last value (should be 100 b/c no change)
```

Now, let's look at how individuals moved between classes using a timeseries plot.
```{r}
# 
plot(x = tset, y = N.simu1, #base plot - population size (vector) on y and time on x
     type = 'l', 
     col = Ncol, 
     las = 1, 
     lwd=2, 
...
#line crossing at 0 so we can see it
abline(h = 0) 

#lines for each of the different classes
lines(x = tset, y = S.simu1, col = Scol, lwd = 2) #susceptible over time
# 
lines(x = tset, y = I.simu1, col = Icol, lwd = 2) #infected over time
# 
lines(x = tset, y = R.simu1, col = Rcol, lwd = 2) #recovered over time

# put a legend on it! 
legend(x = max(tset)*0.1, y = N*1.2, #position of legend 
       lwd = 2, #line type
       legend = c('N', 'S', 'I', 'R'), #label legend
       col = c(Ncol, Scol, Icol, Rcol), #order
       horiz = TRUE) #arrange them horizontally 
```
...
```{r}
sir.fun <- function (t, x, params) { #building our own fxn (intake 3 vectors: t, x, params)
  # population sizes
  #for our named x vector, this is the order
  S <- x[1] 
  I <- x[2]
  R <- x[3]
  N <- x[4]
  ...
  # parameters
  # for params vector, length 2 with beta value and gamma value
  beta <- params[1] 
  gamma <- params[2] 
  
  # change in population size over change in time
  # creating objects with our equations (same as above)
  dSdt <- -beta*S*I
  dIdt <- (beta*S*I)-(gamma*I)
  dRdt <- gamma*I
  dNdt <- dSdt+dIdt+dRdt
  #set up differently here because we dont have *dt (not specifying period of time between each time ODT solver steps in)
  
  # function outputs
  return(list(c(dSdt, dIdt, dRdt, dNdt))) #return a list of our simulated classes
}
```
...
# vector of parameters (because ODE solver wants this/we set this in ODE solver)
parameters <- c(beta, gamma)
```

The last thing we'll do is create our time vector, called `times`.
```{r}
times <- seq(from = 0, to = 60, length.out = 20000) #amount of time for simulation
```
...
```{r}
results <- lsoda(initial_values, times, sir.fun, parameters) #function
```

We can also double check the output by using `head()`, as before.
```{r}
head(results) #show up first chunk of results
```
...
```{r}
#create timeseries then plot our populations

# plot N from ODE solver
plot(x = results[, "time"], y = results[, "N"], #indexing from object of results (x - want all the rows but only the column time; y - all the results but just from N column)
     type = 'l', col = Ncol, las = 1, lwd = 2, # line modifications
     ylim = c(0, N*1.2), # y-axis limits
     xlab = 'Time', ylab = 'Number of Individuals', # labels
     main = "ODE Solver")
...
lines(x = results[, "time"], y = results[, "S"], col = Scol, lwd = 2) #to plot S, index S
lines(x = results[, "time"], y = results[, "I"], col = Icol, lwd = 2) #to plot I, index I
lines(x = results[, "time"], y = results[, "R"], col = Rcol, lwd = 2) #to plot R, index R
...
```{r}
#timeseries of folks that are infected
# plot I.simu1 from for() loop
plot(x = tset, y = I.simu1, #output from for() loop
     type = 'l', las = 1, col = Icol, lwd = 3, #line type and color
     xlab = 'Time', ylab = 'Incidence') #labels

# plot I from ODE solver
lines(x = results[, "time"], y = results[, "I"], #from ODE output
      col = 'black', lwd = 1, lty = 2) #line type and color

# add a legend
legend(x = max(tset)*0.6, y = max(I.simu1)*0.9, #position of legend
       lwd = 2, #line type
       legend = c('For() Loop', 'ODE Solver'), #labels
       col = c(Icol, 'black'), #color of lines
...
plot(x = tset, y = (I.simu1+R.simu1)/(N.simu1),
     ...
     xlab = "time", ylab = "proportion of prev or currently sick")
...
```{r}
#new infections over time (beta si)
plot(x = tset, y = (beta*S.simu1*I.simu1),
     type = 'l', col = Icol, lwd = 3,
     xlab = "time", ylab = "number of new infections")

# max <- max(beta*S.simu1*I.simu1)
# max
# timemax <- tset[max]
# timemax

#create vector of values for new infections
newinfect <- beta*S.simu1*I.simu1
maxnewinfect <- max(newinfect)

#need to pull the max value 
for(i in 1:length(tset)) {
  if (newinfect[i] == maxnewinfect) {
    Time_max_ni <- tset[i]
  }
}
Time_max_ni
...
```{r}
# setting up vaccinated class = number in pop * vaxx rate
V <- N*p 

# set up initial conditions for susceptible
...
for(i in 2:length(tset)){
  # 
	...
	# setting these as holding for simu values
	S <- S.simu2[i-1]
	I <- I.simu2[i-1]
	R <- R.simu2[i-1]
	...
	# adding recent change to delta t
	S.simu2[i] <- S + dS
	I.simu2[i] <- I + dI
	R.simu2[i] <- R + dR
	N.simu2[i] <- N + dN
...
vaccination reduced incidence / spike in infections, later peak in infections
...
# plot I.simu2 from the 70% vaccinated population
lines(x = tset, y = I.simu2, col = 'black', lwd = 2)
lines(x = tset, y = I.simu3, col = 'red', lwd = 2)

# add a legend
legend(x = max(tset)*0.6, y = max(I.simu1)*.9, 
       lwd = 2,
       legend = c('Unvacc.', '70% Vacc', '80% Vacc'),
       col = c(Icol, 'black', 'red'))
...
Since on new doc I had to reinput the ODE solver
...
# initial number infected (setting this assuming it is 1)
I0 <- 1 
...
###########################################################
betablack <- 0.01   # infection rate
gammablack <- 0.1  # recovery rate
parameters1 <- c(betablack, gammablack) #vector of parameters, naming them

#Now we're ready to run `lsoda()`! If you're curious about the arguments it takes, check out the help page (`?lsoda`). We'll store the output as the variable `results`.
resultshw1 <- lsoda(initial_values, times, sir.fun, parameters1) #function


##########################################################
# 1b. With a blue line: beta = 0.05, gamma = 0.1, p = 0 #
##########################################################
betablue <- 0.05   # infection rate
gammablue <- 0.1  # recovery rate
parameters2 <- c(betablue, gammablue)

resultshw2 <- lsoda(initial_values, times, sir.fun, parameters2) #function

###########################################################
# 1c. With a green line: beta = 0.01, gamma = 0.3, p = 0 #
###########################################################
betagreen <- 0.01   # infection rate
gammagreen <- 0.3  # recovery rate
parameters3 <- c(betagreen, gammagreen) #vector of parameters, naming them

resultshw3 <- lsoda(initial_values, times, sir.fun, parameters3) #function

```

now plotting
```{r}
plot(x = resultshw1[, "time"], y = results[, "I"], #indexing from object of results (x - want all the rows but only the column time; y - all the results but just from N column)
     type = 'l', col = "black", las = 1, lwd = 2, # line modifications
     ylim = c(0, N), # y-axis limits
     xlab = 'Time', ylab = 'Incidence', # labels
     main = "Incidence of Infections")

# add a horizontal line at 0
abline(h = 0)

# plot S, I, and R from ODE solver
lines(x = resultshw2[, "time"], y = resultshw2[, "I"], col = "blue", lwd = 2) #to plot I, index I
lines(x = resultshw3[, "time"], y = resultshw3[, "I"], col = "green", lwd = 2) #to plot I, index I

# add a legend
legend(x = max(tset)*0.2, y = N, 
       lwd= 2, 
       legend = c('beta = 0.01, gamma = 0.1', 'beta = 0.05, gamma = 0.1', 'beta = 0.01, gamma = 0.3'),
...
Decreasing infectiousness (beta - comparing blue vs black lines) resulted in a less steep increase infections initially, with a less steep decline as time went on. It "flattened the curve" so to speak and resulted in lower peak of number of infections overall as well. 

## 2b. What is the effect of increasing recovery rates? 

Increasing recovery rate (gamma - comparing black and green lines) led to a steeper dropoff of the infected individuals over time. A higher gamma - recovery rate, also resulted in a lower max incidence rate, further flattening the curve, despite a similar infectiousness rate. This is liekly beacuase poeple were recovering faster, so not spreading the infection for as long. 
...
#R0 = beta*S/gamma
#Rv = R0(1-p)
#assuming Rv = 1 (preventing outbreak) we know then p = 1 - 1/R0

R1a <- (betablack*S)/gammablack
p1a <- 1 - 1/R1a

R1b <- (betablue*S)/gammablue
p1b <- 1 - 1/R1b

R1c <- (betagreen*S)/gammagreen
p1c <- 1 - 1/R1c
...
**The vaccination proportion (p_crit) for the "black line" scenario 1 is: 0.9**

**The vaccination proportion (p_crit) for the "blue line" scenario 2 is: 0.98**

**The vaccination proportion (p_crit) for the "green line" scenario 3 is: 0.7**
...
# set up initial conditions for susceptible
S0 <- N - Vhw41 - I0 # HINT: Why is this the same as N*(1-p)-I0?

# holding vectors and initial conditions
N.simu41 <- NaN*tset; N.simu41[1] <- N
S.simu41 <- NaN*tset; S.simu41[1] <- S0
I.simu41 <- NaN*tset; I.simu41[1] <- I0
R.simu41 <- NaN*tset; R.simu41[1] <- N - I0 - S0 - Vhw41
# 
for(i in 2:length(tset)){
  # 
	dt <- tset[i]-tset[i-1]
	...
	# equations
	dS <- (-betablack*S*I)*dt
	dI <- (betablack*S*I-gammablack*I)*dt
	dR <- (gammablack*I)*dt
	dN <- dS+dI+dR  
...
plot(x = resultshw1[, "time"], y = resultshw1[, "I"], #indexing from object of results (x - want all the rows but only the column time; y - all the results but just from N column)
     type = 'l', col = "black", las = 1, lwd = 2, # line modifications
     ylim = c(0, N-20), # y-axis limits
     xlab = 'Time', ylab = 'Incidence', # labels
     main = "Comparing Vaccination with beta = 0.01, gamma = 0.1")

# add a horizontal line at 0
abline(h = 0)

# plot S, I, and R from ODE solver
lines(x = tset, y = I.simu41, col = 'blue', lwd = 2) #infected over time

# add a legend
legend(x = max(tset)*0.2, y = N-20, 
       lwd= 2, 
       legend = c('Unvaccinated (p=0)', 'Vaccinated (p=0.9)'),
       col = c( "black", "blue"))
...
# set up initial conditions for susceptible
S0 <- N - Vhw42 - I0 # HINT: Why is this the same as N*(1-p)-I0?

# holding vectors and initial conditions
N.simu42 <- NaN*tset; N.simu42[1] <- N
S.simu42 <- NaN*tset; S.simu42[1] <- S0
I.simu42 <- NaN*tset; I.simu42[1] <- I0
R.simu42 <- NaN*tset; R.simu42[1] <- N - I0 - S0 - Vhw42
# 
for(i in 2:length(tset)){
  # 
	dt <- tset[i]-tset[i-1]
	...
	# equations
	dS <- (-betablue*S*I)*dt
	dI <- (betablue*S*I-gammablue*I)*dt
	dR <- (gammablue*I)*dt
	dN <- dS+dI+dR  
...
plot(x = resultshw2[, "time"], y = resultshw2[, "I"], #indexing from object of results (x - want all the rows but only the column time; y - all the results but just from N column)
     type = 'l', col = "black", las = 1, lwd = 2, # line modifications
     ylim = c(0, N), # y-axis limits
     xlab = 'Time', ylab = 'Incidence', # labels
     main = "Comparing Vaccination with beta = 0.05, gamma = 0.1")

# add a horizontal line at 0
abline(h = 0)

# plot S, I, and R from ODE solver
lines(x = tset, y = I.simu42, col = 'blue', lwd = 2) #infected over time

# add a legend
legend(x = max(tset)*0.2, y = N-20, 
       lwd= 2, 
       legend = c('Unvaccinated (p=0)', 'Vaccinated (p=0.98)'),
       col = c( "black", "blue"))
...
# set up initial conditions for susceptible
S0 <- N - Vhw43 - I0 # HINT: Why is this the same as N*(1-p)-I0?

# holding vectors and initial conditions
N.simu43 <- NaN*tset; N.simu43[1] <- N
S.simu43 <- NaN*tset; S.simu43[1] <- S0
I.simu43 <- NaN*tset; I.simu43[1] <- I0
R.simu43 <- NaN*tset; R.simu43[1] <- N - I0 - S0 - Vhw43
# 
for(i in 2:length(tset)){
  # 
	dt <- tset[i]-tset[i-1]
	...
	# equations
	dS <- (-betagreen*S*I)*dt
	dI <- (betagreen*S*I-gammagreen*I)*dt
	dR <- (gammagreen*I)*dt
	dN <- dS+dI+dR  
...
plot(x = resultshw3[, "time"], y = resultshw3[, "I"], #indexing from object of results (x - want all the rows but only the column time; y - all the results but just from N column)
     type = 'l', col = "black", las = 1, lwd = 2, # line modifications
     ylim = c(0, N-60), # y-axis limits
     xlab = 'Time', ylab = 'Incidence', # labels
     main = "Comparing Vaccination with beta = 0.1, gamma = 0.03")

# add a horizontal line at 0
abline(h = 0)

# plot S, I, and R from ODE solver
lines(x = tset, y = I.simu43, col = 'green', lwd = 2) #infected over time

# add a legend
legend(x = max(tset)*0.2, y = N-60, 
       lwd= 2, 
       legend = c('Unvaccinated (p=0)', 'Vaccinated (p=0.70)'),
       col = c( "black", "green"))
...
# plot of $R_0$ (y-axis) vs. Infectiousness (beta - x-axis)

beta_set <- seq(from = 0.001, to = 0.1, length.out = 100) #setting vector for beta

# equation for R0 for the time set from beta 
R0_set <- beta_set*N/gamma

#the plot
plot(x = beta_set, y = R0_set, #setting up axes 
     type = 'l', las = 1, lwd = 2, #line type and thickness
     xlab = 'Infectiousness (beta)', ylab = 'Basic Reproductive Number (R_0)') #axis labels

# plot of $p$ (y-axis) vs. Infectiousness (beta - x-axis)
p_crit_set <- 1 - 1/R0_set  #adapting Rv equation to get p based on R0 from above

#the plot
plot(x = beta_set, y = p_crit_set, #setting up axes
     type = 'l', las = 1, lwd = 2, #line type
     xlab = 'Infectiousness (beta)', ylab = 'Critical Vaccination Rate (p_crit)') #label axes

```

explain the shapes of the graphs based on the biology of disease spread

**If R0 is the basic reproductive ratio of the disease, for a new population this depends on the rates of beta and gamma, and the number of individuals in a population. it makes sense that infectiousness would relate linearly to basic reproductive number because as the ability for a disease to spread, rate of infectiousness would also have to increase and vice versa. When you have vaccination in a population since p represents the proportion of people vaccinated, the figure shows that as a disease is more infectious, you have more people vaccinated. Then you reach a plateau after which herd immunity kicks in and despite an increase of infectiousness, no more people are vaccinated. The plateau is below 1 because getting 100% of the population vaccinated isnt representative of realistic scenarios.**
...
# plot of $R_0$ (y-axis) vs. Recovery time (gamma - x-axis)
# create a vector of values for N over which to calculate R0
gammaset <- seq(from = 0, to = 100, length.out = 100)

# calculate R0 for each value of N
R0_set.gamma <- beta*N/gammaset

# plot R0 as a function of gamma
plot(x = gammaset, y = R0_set.gamma, 
     type = 'l', las = 1, lwd = 2,
     xlab = 'Recovery time (gamma)', ylab = 'Basic Reproductive Number (R_0)')

# plot of $p$ (y-axis) vs. Recovery time (gamma - x-axis)
p_crit_setgamma <- 1 - 1/R0_set.gamma  #adapting Rv equation to get p based on R0 from above

#the plot
plot(x = gammaset, y = p_crit_setgamma, #setting up axes
     type = 'l', las = 1, lwd = 2, #line type
     xlab = 'Recovery time (gamma)', ylab = 'Critical Vaccination Rate (p_crit)') #label axes
...
**Reproductive number fo the disease decreases with increasing recovery time beacuse as people overcome the disease, they enter a class that cant get infected again so the disease can't have in increasing reproductive ratio. Since gamma is on the bottom of the R0 equation, this mathmatically makes sense as well (R0 decreases as gamma increases). Vaccination rate would also intrinsically decrease as recovery time inrceases beacuse if there is a quick turnaround within an individual for a disease, the likelihood you need to vaccinate would go down. If you have a quick recovery time, you are probably not infectious for a while, so the disease might not be as prevalent to spread and you see vaccination go down.**
...
# plot of $R_0$ (y-axis) vs. population size (x-axis - N)
# create a vector of values for N over which to calculate R0
Nset <- seq(from = 0, to = 100, length.out = 100)
...
# plot R0 as a function of N
plot(x = Nset, y = R0_set.N, 
     type = 'l', las = 1, lwd = 2,
     xlab = 'Population size (N)', ylab = 'Basic Reproductive Number (R_0)')

# plot of $p$ (y-axis) vs. population size (x-axis - N)
p_crit_setN <- 1 - 1/R0_set.N  #adapting Rv equation to get p based on R0 from above

#the plot
plot(x = Nset, y = p_crit_setN, #setting up axes
     type = 'l', las = 1, lwd = 2, #line type
     xlab = 'Population size (N)', ylab = 'Critical Vaccination Rate (p_crit)') #label axes
...
**Basic reproductive number increases as population size inrceases because as a disease reaches larger populations it has larger potential to spread and therefore higher reproductive potential. The pcrit values here are negative, reflecting their calculated equation where R0 must be close to or less than 1 (approaching 1 as N increases) because of the R0 equation which depends on beta*N/gamma. So here, as N is small, R0 must be less than 1, resulting in negative pcrit. Since a disease wont really break out when R0 is less than 1 (spread occurs when R0 > 1), these negative pcrit values refelct that its biologically unimportant to evaluate pcrit when a disease is not spreading because there is potentially no need to vaccinate for a disease that doesnt spread.**
