...
Patch 1: the host population starts at k and stays for the whole simulation.
Host emerges later in later patches as you go from 5 to 15. Each one eventually reaches K. 
...
a. Make two plots that compare the host (plot 1) and partner's (plot 2) invasion fronts on Day 1000 for (a) D_H = D_P (synchronous), (b) D_H > D_P (host first), and (c) D_H < D_P (partner first). Be sure to include a legend!  
...
plot(x = Xset, y = H.simu4[1000,], 
     type = 'l', lwd = 2, col = Hcol, 
     xlab = 'Location', ylab = 'Host Population Size', 
     main = 'Host pop sizes, different order of mutualist introduction (t = 1000)')
lines(x = Xset, y = H.simu5[1000,],
      lwd = 2, col = 'seagreen3')
lines(x = Xset, y = H.simu6[1000,],
      lwd = 2, col = 'seagreen2')
#adding legend
legend(x = 7, y = 35, 
       legend = c('H with same speed mutualist (D_H = D_P)','H with faster host (D_H > D_P)','H with with faster mutualist (D_H < D_P)'), 
       lwd = 2, 
...
plot(x = Xset, y = P.simu4[1000,], 
     type = 'l', lwd = 2, col = Pcol, 
     xlab = 'Location', ylab = 'Partner Population Size', 
     main = 'Partner pop sizes, different order of mutualist introduction (t = 1000)')
lines(x = Xset, y = P.simu5[1000,],
      lwd = 2, col = 'lightblue4')
lines(x = Xset, y = P.simu6[1000,],
      lwd = 2, col = 'lightblue3')
...
# add a legend
legend(x = 7, y = 35, 
       legend = c('P with same speed mutualist (D_H = D_P)','P with faster host (D_H > D_P)','P with with faster mutualist (D_H < D_P)'),
       lwd = 2, 
       col = c(Pcol,'lightblue4','lightblue3'),
       bg = 'white')
```
...
**Host-first dispersal (Goes the furthest)**

= /2 points total  

c. Why does (or does not) a faster partner dispersal rate accelerate invasion?  

**Host-first dispersal accelerates the invasion because the partner needs the host to survive so presence of the host already in the system creates favorable conditions for partner survival. Host-first also moves further over time (progressing into further away patches by day 3000) and the steep slope of the partner first and synchronous invasinos show it quickly goes to a population of zero.**
...
plot(x = Xset, y = H.simu.par2[1000,], 
     type = 'l', lwd = 2, col = Hcol, 
     xlab = 'Location', ylab = 'Host Population Size', 
     main = 'Host pop sizes, different order of parasite introduction (t = 1000)')
...
# all partner population sizes at t = 1000
plot(x = Xset, y = P.simu.par2[1000,], 
     type = 'l', lwd = 2, col = Pcol, 
     xlab = 'Location', ylab = 'Partner Population Size', 
     main = 'Parasite pop sizes, different order of parasite introduction (t = 1000)')
...
# add a legend
legend(x = 10, y = 10, 
       legend = c('Day 1000, P (D_H = D_P)','Day 1000, P (D_H > D_P)','Day 1000, P (D_H < D_P)'), 
       lwd = 2, 
       col = c(Pcol ,'lightblue3','lightblue2'),
       bg = 'white')
...
**Host-first dispersal appears to be the fastest invasion (the lines furthest to the right of the figure that progress the furthest)**
...
# plot H at time 1000 for agent A
plot(x = Xset, y = H.simu.aga[1000,], 
     type = 'l', lwd = 2, col = Hcol, 
     xlab = 'Location', ylab = 'Host Population Size', 
     main = 'Invasion front of agents A - C, days 1000 - 3000',
     ylim = c(0, 17), las = 1)
# plot H agent A at time 2000
lines(x = Xset, y = H.simu.aga[2000,],
      lwd = 2, col = 'seagreen2')
# plot H agent A at time 3000
lines(x = Xset, y = H.simu.aga[3000,],
      lwd = 2, col = 'seagreen1')
# plot H agent b at time 1000
lines(x = Xset, y = H.simu.agb[1000,],
      lwd = 2, col = 'red')
# plot H agent b at time 2000
lines(x = Xset, y = H.simu.agb[2000,],
      lwd = 2, col = 'red3')
# plot H agent b at time 3000
lines(x = Xset, y = H.simu.agb[3000,],
      lwd = 2, col = 'red4')
# plot H agent c at time 1000
lines(x = Xset, y = H.simu.agc[1000,],
      lwd = 2, col = 'lightblue')
# plot H agent c at time 2000
lines(x = Xset, y = H.simu.agc[2000,],
      lwd = 2, col = 'lightblue3')
# plot H agent c at time 3000
...
# add a legend
legend(x = 15, y = K*.9, 
       legend = c('Day 1000, A', 'Day 2000, A', 'Day 3000, A', 'Day 1000, B', 'Day 2000, B', 'Day 3000, B', 'Day 1000, C', 'Day 2000, C', 'Day 3000, C'), 
       lwd = 2, 
       col = c(Hcol, 'seagreen2', 'seagreen1', 'red', 'red3', 'red4', 'lightblue', 'lightblue3', 'lightblue4'),
       bg = 'white')

#not sure why this is beign weird
```

### Just to see some others:
...
legend(x = 15, y = K*.9, 
       legend = c('Day 0', 'Day 1000', 'Day 2000', 'Day 3000'), 
       lwd = 2, 
       col = c(Pcol, 'lightblue', 'lightblue3', 'lightblue4'),
       bg = 'white')
